...
import {
    Server, HardDrive, Network, Shield, Box
} from 'lucide-react';
...
export const Ec2InstanceDetails: React.FC<Ec2InstanceDetailsProps> = ({ instance, node, clusterName }) => {
    ...
    useEffect(() => {
        ...
        if (!nodeName || !clusterName) return;

        const fetchNodePods = async () => {
            setLoadingPods(true);
            try {
                const allPods = await window.k8s.getPods(clusterName);
                const nodePods = allPods.filter((p: any) => p.node === nodeName || p.spec?.nodeName === nodeName);
                setPods(nodePods);
            } catch (err) {
                                    ...
                                    <div className="border border-white/10 rounded-lg overflow-hidden bg-black/20">
                                        <table className="w-full text-left text-sm">
                                            ...
                                            <tbody className="divide-y divide-white/5">
                                                {pods.map((pod, idx) => (
                                                    <tr key={pod.metadata?.uid || idx} className="hover:bg-white/5">
                                                        <td className="p-3 text-gray-300">{pod.namespace || pod.metadata?.namespace}</td>
                                                        <td className="p-3 font-mono text-blue-300">{pod.name || pod.metadata?.name}</td>
                                                        <td className="p-3">
                                                            <span className={`px-1.5 py-0.5 rounded text-xs ${(pod.status === 'Running' || pod.status?.phase === 'Running') ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'
                                                                }`}>
                                                                {pod.status?.phase || pod.status}
                                                            </span>
                                                        </td>
                                                    </tr>
                                                ))}
