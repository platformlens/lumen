...
export const AwsView: React.FC<AwsViewProps> = ({ clusterName, onResourceClick }) => {
    ...
    const podIdentityColumns: IColumn[] = React.useMemo(() => [
        { label: 'Namespace', dataKey: 'namespace', width: 150, sortable: true },
        {
            ...
            dataKey: 'serviceAccount',
            width: 200,
            sortable: true,
            cellRenderer: (val: any) => (
                <span className="text-blue-400 cursor-pointer hover:underline">{val}</span>
            ),
        },
    ...
    const getMatchingNode = (instance: any) => {
        ...
        return clusterNodes.find(node => {
            ...
            const nodeHostname = (node.status?.addresses || []).find(
                (a: any) => a.type === 'Hostname'
            )?.address || '';

            // Match by providerID containing instance ID
            if (instanceId && providerId.includes(instanceId)) return true;

            // Match by private DNS name (node name is often the private DNS)
        ...
        <>
            <div className="p-6 space-y-8 pb-20">
                ...
                <section>
                    ...
                    <PagedTable
                        ...
                        onRowClick={(row) => {
                            ...
                            const node = getMatchingNode(row);
                            onResourceClick({ ...row, _matchedNode: node }, 'ec2instance');
                        }}
                    />
                </section>
                ...
                <section>
                    ...
                    <PagedTable
                        tableId="aws-pod-identities"
                        data={podIdentities}
                        columns={podIdentityColumns}
                        onRowClick={async (row) => {
                            if (!onResourceClick) return;
                            try {
                                const sa = await window.k8s.getServiceAccount(clusterName, row.namespace, row.serviceAccount);
                                if (sa) {
                                    onResourceClick({ ...sa, name: sa.metadata?.name, namespace: sa.metadata?.namespace, type: 'serviceaccount' }, 'serviceaccount');
                                }
                            } catch (e) {
                                console.warn('Failed to fetch service account:', e);
                            }
                        }}
                    />
                </section>
