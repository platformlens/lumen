...
export class TerminalService {
    ...
    createExecTerminal(sender: Electron.WebContents, id: string, cols: number, rows: number, context: string, namespace: string, podName: string, containerName?: string) {
        ...
        try {
            ...
            let execCmd = `kubectl exec -it --context=${context} -n ${namespace} ${podName}`;
            if (containerName) {
                execCmd += ` -c ${containerName}`;
            }
            execCmd += ` -- /bin/sh`;

            console.log(`[Terminal] Exec command: ${execCmd}`);

            // Spawn shell and immediately write the exec command
            const args = os.platform() === 'win32' ? [] : ['--login'];
            const ptyProcess = pty.spawn(shell, args, {
                name: 'xterm-256color',
                cols: cols || 80,
            ...
            // Write the exec command to the terminal
            ptyProcess.write(`${execCmd}\r`);
